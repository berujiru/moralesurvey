...
class Suggestion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('suggestiontext', 'required'),
			array('suggestby', 'numerical', 'integerOnly'=>true),
			array('datefiled', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, datefiled, suggestiontext, suggestby', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('suggestiontext',$this->suggestiontext,true);
		$criteria->compare('suggestby',$this->suggestby);
	...
	{
	    $this->suggestby = Yii::app()->user->id;
