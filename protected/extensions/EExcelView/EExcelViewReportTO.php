<?php
	...
	* @version 0.3
	*/	
	class EExcelViewReportTO extends CGridView
	{
		//Document properties
		...
		public function init()
		{
			...
			{			
				...
				$this->objPHPExcel = PHPExcel_IOFactory::load("./protected/data/draft_TO.xlsx");
		...
		public function renderRow($row)
		{
			...
			//this is a customize, we comment out the rest cause we will just targeting the cells to populate with data
			//the current date
			

			//get the owner of the request
			$owner = Profile::model()->findByPk($data['0']['user_id']);
			$this->objPHPExcel->getActiveSheet()->setCellValue('A14',$owner->firstname." ".$owner->lastname);

			$posdata = Position::model()->findByAttributes(array('user_id'=>$data['0']['user_id']));
			//position
			if($posdata){
				if($posdata->position_type==="regular"){
					//get from plantilla tbl
					$posdata2=ReferPositionPlantilla::model()->findByPk($posdata->position_id);
					$this->objPHPExcel->getActiveSheet()->setCellValue('D14',$posdata2->plantillaName);
				...
				elseif($posdata->position_type==="contractual"){
					...
					$this->objPHPExcel->getActiveSheet()->setCellValue('D14',$posdata2->position_name);
				}
			}

			//get the division
				$tempdiv = Division::model()->findByAttributes(array('user_id'=>$data['0']['user_id']));
				$tempdiv = ReferDivision::model()->findByPk($tempdiv->division_id);
				$this->objPHPExcel->getActiveSheet()->setCellValue('F14',$tempdiv->division_code);



			//names of whoswithyou
			// $this->objPHPExcel->getActiveSheet()->setCellValue('A15',$data['0']['whosWithYou']);
			$customrow = 15;
			// $other = explode(',',$data['0']['whosWithYou']);
			// foreach ($other as $person) {
			// 	if($person){
			// 		//output the name

			// 		$this->objPHPExcel->getActiveSheet()->setCellValue('A'.$customrow,$person);
			// 	}
			// 	//get the position
			// 	$posdata = Position::model()->findByAttributes(array('user_id'=>Yii::app()->user->id));

			// 	if($posdata){
			// 		if($posdata->position_type==="regular"){
			// 			//get from plantilla tbl
			// 			$posdata2=ReferPositionPlantilla::model()->findByPk($posdata->position_id);
			// 			$this->objPHPExcel->getActiveSheet()->setCellValue('D'.$customrow,$posdata2->plantillaName);
			// 			//also get the salary data
			...
			// 			//get form contractual tbl
			// 			$posdata2=ReferPositionContract::model()->findByPk($posdata->position_id);
			// 			$this->objPHPExcel->getActiveSheet()->setCellValue('D'.$customrow,$posdata2->position_name);
			// 		}
			// 	}
			...
			// 	//$this->objPHPExcel->getActiveSheet()->setCellValue('C'.$customrow,$position);
			// 	$customrow++;
			// }

			foreach($data['0']['whosWithYou'] as $id) {
				if($id){
				  $tmpdata = Profile::model()->findByPk($id);
				  $person =$tmpdata->firstname." ".$tmpdata->lastname.",";
				...
				//get the division
				$tempdiv = Division::model()->findByAttributes(array('user_id'=>$id));
				$tempdiv = ReferDivision::model()->findByPk($tempdiv->division_id);
				$this->objPHPExcel->getActiveSheet()->setCellValue('F'.$customrow,$tempdiv->division_code);
				$customrow++;
			}

			$this->objPHPExcel->getActiveSheet()->setCellValue('H6',Date('Y-M-d'));

			$this->objPHPExcel->getActiveSheet()->setCellValue('A22',$data['0']['destination']);
			$this->objPHPExcel->getActiveSheet()->setCellValue('D22',Yii::app()->dateFormatter->format("yyyy-MMM-d",strtotime($data['0']['dateFrom']))." to ".Yii::app()->dateFormatter->format("yyyy-MMM-d",strtotime($data['0']['dateTo'])));
			$this->objPHPExcel->getActiveSheet()->setCellValue('F22',$data['0']['purpose']);
			// $a=0;
